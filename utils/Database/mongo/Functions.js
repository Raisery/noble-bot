...
import Profanity from './models/profanity.js'
import Song from './models/songs.js'
import Logger from '../../Logger.js'

export default function Functions(client) {
	...
	client.updateAnnonceInBDD = async (user, guild, song_path) => {
		...
		if (!annonceData) {
			annonceData = await client.createAnnonceInBDD(user, guild, song_path)
		} else {
	...
	client.updateGuildInBDD = async (guild, { duration_limit = null, ignored_voice_channels = null }) => {
		...
		if (!ignored_voice_channels) ignored_voice_channels = guildData.ignored_voice_channels
