					...
					options: [
						{
							name: 'value',
							description:
								'Nouvelle valeur de durationLimit en format <minute:secondes> (00:20)',
	...
	run: async (client, interaction) => {
			...
			if (interaction.options._subcommand === 'set') {
				...
				if (durationLimit.match(format)) {
					...
					await interaction.reply(
						`Format de valeur incorrecte -> la valeur doit être dans le format MINUTES:SECONDES ( ex: 04:23)`
					)
				}
			}
			return
		...
		if (interaction.options._group === 'ignored_vc') {
			...
			if (interaction.options._subcommand === 'add') {
				...
				if (!VC || VC.type != ChannelType.GuildVoice)
					return interaction.reply('Tu dois entrer un voice chat valide')
				const guildData = await client.getGuildFromBDD(interaction.guild)
		...
		if (interaction.options._group === 'custom_bad_words') {
			...
			if (interaction.options._subcommand === 'add') {
				...
				await interaction.reply(
					`Nouvelle valeur de customBadWords : ${profanityParam.customBadWords}`
				)
			}
			if (interaction.options._subcommand === 'remove') {
				const value = interaction.options.getString('badword')
