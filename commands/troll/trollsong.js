...
import download from '../../utils/tools/download.js'
...
const trollsong = {
	...
	options: [
		{
			name: 'add',
			description: "Envoi un son au format MP3 pour pouvoir l'utiliser avec le bot",
		...
		{
			name: 'delete',
			description: 'Supprime un son de la liste (ATTENTION : cette action supprime le fichier audio !)',
			type: ApplicationCommandOptionType.Subcommand,
...
async function trollSong_add(client, interaction) {
	...
	const downloaded = await download(son.attachment.url, interaction.guild, 'trolls', titre.value)
	if (downloaded) {
		const song = {
			...
			path: './storage/' + interaction.guild.id + '/trolls/' + titre.value + '.mp3',
		}
		client.createSongInBDD(interaction.user, interaction.guild, song)
		reply(interaction, titre.value + '.mp3 téléchargé avec succés !')
	} else {
		reply(interaction, titre.value + '.mp3 existe déjà! Choisis un autre nom!')
	}
}
